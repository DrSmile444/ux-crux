...
The general error-prevention/recovery rules (state preservation on failure, idempotent retries, recovering without redoing unrelated work) are covered by the separate `usability` skill, not repeated here. This file covers only the confirmation-and-consent angle of destructive actions.
