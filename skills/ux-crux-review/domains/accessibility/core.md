...
| X13 | Instructional or help microcopy describes the order of actions to take (chronological: "Next, select OK") rather than an element's visual/spatial position ("the button below", "the panel on the right"), since assistive technology reads content in document order and responsive layouts reposition elements. Distinct from X02 (SC 1.4.1, color alone): this rule covers spatial/positional language, not color. | Accessibility | Major | WCAG 2.2 Sensory Characteristics (SC 1.3.3) |
| X14 | Call-to-action and instructional copy prefers a device-agnostic action verb ("Select", "Choose", "View") over a hardware-specific one ("Click", "Tap", "Press"), since interfaces are operated by mouse, touch, voice, and switch-access devices. Does not apply when copy is deliberately teaching a specific physical gesture the interface requires (for example "Pinch to zoom out"). | Strong | Moderate | Apple HIG Accessibility; WCAG 2.2 Sensory Characteristics (SC 1.3.3) |
| X15 | A design's assumptions about hardware capability, network connectivity, and digital literacy are checked against Kranzberg's First Law of Technology ("technology is neither good nor bad, nor is it neutral"): a product that fails, degrades unacceptably, or becomes unusable under low-bandwidth connectivity, on older or budget-tier devices, or without high-speed continuous connectivity is flagged when its plausible user base includes users in those conditions. Distinct from X01-X14 (which address disability-focused WCAG conformance): X15 addresses equitable access across economic/infrastructure conditions, a separate inclusive-design axis. Does not apply when the evidence establishes the product is a specialized enterprise/professional tool with a legitimate, stated minimum hardware/connectivity requirement for its verified audience. | Strong | Major | Melvin Kranzberg, "Technology and History: Kranzberg's Laws"; danah boyd, digital-inequality research (via Mark Wells, *User Experience Design*) |
...
Always identify target platform(s) before scoring a target-size or text-scaling finding (X04/X05/T01/T02 live in `mobile.md`) — the correct minimum differs by platform, and applying iOS's 44pt to an Android screen (or vice versa) is itself a wrong finding. WCAG-sourced rules (X01-X03, X06-X14, T03-T05) apply regardless of platform and should be checked first since several carry Blocker severity. When evidence is a static screenshot or mockup, most of this file's rules are only partially assessable — contrast (X03), color-alone cues (X02), and instructional/action-verb wording (X13-X14) can be checked from a static image or copy alone, but reading order (X01), focus visibility (X08), and gesture-only functionality (X06, T05) usually require a running build or code; report accordingly using the evidence model.

Check X15 whenever the evidence indicates or implies the product's real-world user base spans varied hardware tiers or connectivity conditions (a consumer app, a public-sector or widely-used service) — this is a distinct inclusive-design axis from X01-X14's disability-focused WCAG conformance, so a product can fully pass WCAG while still failing X15's equitable-access concern. Do not apply X15 to a specialized enterprise/professional tool whose evidence establishes a legitimate minimum hardware/connectivity requirement for its verified audience.
